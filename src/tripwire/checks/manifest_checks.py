...
from tripwire.ast_helpers import _skip
...
def check_dependency_cve(path: Path, lines: list[str], tree: ast.Module | None) -> list[Finding]:
    ...
    vuln_deps: list[tuple[str, str, str]] = [
        ("urllib3<1.26.19", "urllib3 <1.26.19", "CVE-2024-37891 — HTTP redirect race condition (CVSS 6.1)"),
        ("requests<2.32.0", "requests <2.32.0", "CVE-2024-35195 — content-length mismatch (CVSS 5.9)"),
        ("cryptography<42.0.0", "cryptography <42.0.0", "CVE-2024-26131 — null-pointer dereference in X.509 (CVSS 5.5)"),
        ("jinja2<3.1.5", "jinja2 <3.1.5", "CVE-2025-27516 — SSTI via template filename injection (CVSS 7.5)"),
        ("werkzeug<3.0.6", "werkzeug <3.0.6", "CVE-2024-49766 — DoS via malicious multipart data (CVSS 7.5)"),
        ("dask<2024.8.1", "dask <2024.8.1", "CVE-2024-45187 — remote code execution via untrusted YAML (CVSS 9.8)"),
        ...
        ("aiohttp<3.10.11", "aiohttp <3.10.11", "CVE-2024-52304 — request smuggling via chunked encoding (CVSS 7.5)"),
        ("fsspec<2024.10.0", "fsspec <2024.10.0", "CVE-2024-40332 — path traversal in HTTP filesystem (CVSS 6.5)"),
    ]
        ...
        for lineno, line in enumerate(lines, 1):
            if dep_pattern.search(line.lower()):
                ...
                if version_match:
                    ...
                    if tuple(int(x) for x in found_ver.split(".")) < tuple(int(x) for x in max_ver.split(".")):
                        results.append(Finding(
                            cwe_id="CWE-1104", cwe_name="Supply Chain — Dependency with Known Vulnerability",
    ...
    for lineno, line in enumerate(lines, 1):
        ...
        if m:
            ...
            if re.search(r',\s*<\s*', stripped):
                continue
            results.append(Finding(
                cwe_id="CWE-1104", cwe_name="Supply Chain — Unbounded Dependency Pin",
