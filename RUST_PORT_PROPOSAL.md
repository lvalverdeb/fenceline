...
- **Speed at scale** — less central here than for spaghetti, since tripwire's checks are simpler (mostly single regex passes over lines, not multi-pass AST complexity scoring), but still real: 56 checks × every file, each running `ast.parse` for the 13 AST-based checks even when just doing a line scan would do, adds up on large external codebases the way it does for spaghetti's `--config`/`--package` use case.
...
| Test suite | `tests/test_tripwire.py` | 252 / 23 tests | Thin relative to the 56 checks, though improved from 12/52 — see §5 |
...
### 7.7 tripwire grew from 52 to 56 checks the same day as the spikes — a Bandit-catalog gap review, not port work

Separately from this proposal, a review comparing tripwire's rule set against Bandit's actual check catalog (tripwire already cross-references several Bandit IDs in its own docstrings — B110/B112, B507, B613, B614) turned up gaps worth closing, each verified against real workspace code or a purpose-built test before adding:
